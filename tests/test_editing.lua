...
T["[ ] becomes [-]"] = function()
	...
	local result = editing.cycle_checkbox_in_line(line, default_checkbox_states)
	MiniTest.expect.equality(result, "- [-] Write plugin")
end

T["[-] becomes [X]"] = function()
	local line = "- [-] Write plugin"
	local result = editing.cycle_checkbox_in_line(line, default_checkbox_states)
	MiniTest.expect.equality(result, "- [X] Write plugin")
end
...
T["Only the first checkbox is cycled"] = function()
	...
	local result = editing.cycle_checkbox_in_line(line, default_checkbox_states)
	MiniTest.expect.equality(result, "- [-] First [ ] Second")
end
